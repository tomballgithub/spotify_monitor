...
def configure_summary(monkeypatch):
    values = {
        ...
        "SPOTIFY_LIVE_DISAPPEARED_CHECK_INTERVAL": 180,
...
def test_version_output_is_machine_friendly():
    ...
    assert result.returncode == 0
    assert result.stdout.splitlines() == [f"spotify_monitor.py v{monitor.VERSION}"]
...
def test_webhook_summary_is_secret_safe(monkeypatch):
    ...
    # The provider name is not sensitive and stays in the full summary; the URL is masked. Both rows are
    # nested under "Notifications (webhook)" and indented, but values still line up in the shared column
    provider_line = next(line for line in output.splitlines() if "Webhook provider:" in line)
    assert provider_line.index("Discord (enabled)") == 32
    url_line = next(line for line in output.splitlines() if "Webhook URL:" in line)
    assert url_line[32:].startswith("<redacted>")
