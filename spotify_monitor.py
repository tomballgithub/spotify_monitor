...
# do this centrally? sp_track = sp_track + current_match.get('icon', '')
...
#   friend_activity - monitors a followed Spotify user's shared tracks, presence and sessions
...
# The Spotify API sometimes doesn't report a specific public shared playlist for a friend, even
# though a song of theirs is really on it (e.g. when Spotify's "smart shuffle" mixes it into their
# queue). This lets you supply your own list of tracks for a playlist and have it detected locally
# by counting consecutive matching songs, instead of relying solely on what the API reports.
#
# Each entry is a dict with the following keys:
#   name      (required) Display name for the playlist, also used as its key in internal state.
#   filename  (required) Path to a text file with one "Artist - Track" line per song in the
#             playlist (see load_spotify_tracks_from_file()). Reloaded periodically; see 'refresh'.
#   qty_start (required) Number of consecutive songs from this playlist's track list needed in a
#             row before the playlist is considered "detected" (started).
#   qty_end   (required) Number of consecutive songs NOT from this playlist's track list allowed
#             in a row (e.g. from Spotify's smart shuffle) before the playlist is considered
#             "cleared" (ended). Set > 0 to tolerate the occasional off-list song without losing
#             the detection.
#   url       (optional) Playlist URL, used in notification links. Default: "" (no link).
#   icon      (optional) Text/emoji appended to the track name once the playlist is detected.
#             Default: "" (none).
#   override  (optional) If True, immediately treat the playlist as detected the first time any
#             one of its songs is seen this session, instead of waiting for 'qty_start' in a row.
#             Only applies the first time a playlist is matched after (re)starting the tool.
#             Default: OVERRIDE_PLAYLIST_AT_START.
#   notify    (optional) Whether to send an email/notification when this playlist is detected or
#             cleared, in addition to always showing its running count on screen.
#             Default: NOTIFY_PLAYLIST_DETECTED.
#   refresh   (optional) Seconds between reloading 'filename' from disk to pick up track-list
#             changes; 0 disables reloading after the initial load. Default: LOAD_TRACKS_FREQUENCY.
#
# example:
...
LOAD_TRACKS_FREQUENCY = 0            # default 'refresh' for a monitored playlist entry that omits it
OVERRIDE_PLAYLIST_AT_START = False   # default 'override' for a monitored playlist entry that omits it
NOTIFY_PLAYLIST_DETECTED   = False   # default 'notify' for a monitored playlist entry that omits it
...
def classify_recovery_error(error: Any = None, context: str = "runtime", detail: Any = "", target_user_id: Optional[str] = None) -> RecoveryAdvice:
    ...
    if context == "webhook_config":
        return make_recovery_advice("webhook.invalid", "The webhook configuration is invalid" + (f": {safe_detail}" if "WEBHOOK_TEMPLATE" in safe_detail else ""), recovery_fix_with_guide("Check the webhook provider, URL, customization, headers and ntfy access token then run --send-test-webhook", WEBHOOK_GUIDE_URL), False, safe_detail)
...
def _config_allowed_names() -> frozenset[str]:
    ...
    return frozenset(declared | EXTRA_CONFIG_KEYS | COMMENTED_CONFIG_SETTINGS)
...
# Validates one imported sp_dc through token acquisition and the selected activity endpoint
...
def run_browser_cookie_import(browser="firefox", browser_profile=None, cookie_file=None, env_file=None, force=False, interactive=None, input_func=None, config_path=None, target=None, saved_target=None, print_next_steps=True):
    ...
    # The wizard has just written a cookie config and prints its own next steps after the file summary, so the standalone epilogue is skipped there
    ...
    monitor_command = _wizard_action_command(method, "", selected_config, destination, monitor_target)
...
# Reading the activity feed is an idempotent POST with the same retry policy as metadata reads
SESSION.mount(SPOTIFY_LISTENING_ACTIVITY_URL, web_player_adapter)
SESSION.mount(SPOTIFY_PLAYLIST_METADATA_URL, web_player_adapter)
...
# ===========================================================================================
# Monitored-playlist detection
# ===========================================================================================
# Spotify doesn't always report which playlist a friend's song came from (most notably its own
# "smart shuffle" mixes). ADD_PLAYLISTS_TO_MONITOR (see its config comment above) lets a playlist
...
# song-by-song match against that data: current_match (the playlist this song belongs to, if any)
# and previous_match (the same, for the previous song) - both live references into
# monitored_playlists_data, so mutating them updates the shared state directly. Comparing the two
# is what detects a playlist switch. Alongside them, four strings - playlist_message,
# playlist_screen_message, playlist_body_text, playlist_body_html - carry the current "detected" /
# "cleared" status in the four shapes different outputs need (log line, screen-only line, email
# text, email HTML); see reset_playlist_counts(), notify_playlist_detected/cleared() and
# monitored_playlist_detected/cleared() below.
#
# The actual state machine (match -> is this a new playlist or a shuffle exception -> did a
...
# across those sites; the rest intentionally isn't unified further, since collapsing it would blur
# real behavioral differences between the three call sites.
# ===========================================================================================


def build_playlist_status_message(playlist_data):
    """One-line running-count status for a monitored playlist, e.g. "Playlist 'X' Count: 3 (1 via smart shuffle)".

    Returns "" if the playlist hasn't reached 'qty_start' consecutive songs yet (nothing to show).
    If count_overridden is set (find_song_in_playlists jump-started this playlist by name match),
    the displayed count is shifted down by the injected head-start so it doesn't look inflated.
    """
    ...
    print_debug(f"BUILDING PLAYLIST STATUS -> name: {name}, count_start: {count_start}, count_shuffle: {count_shuffle}")
    print_debug(f"BUILDING PLAYLIST STATUS -> shuffle_details: {shuffle_details}")
    print_debug(f"BUILDING PLAYLIST STATUS -> override: {count_overridden}")
    if count_start < playlist_data['qty_start']:
        print_debug(f"CLEARING PLAYLIST STATUS DUE TO START_CNT < QTY_CNT -> name: {name}, count_start: {count_start}, count_end: {playlist_data['qty_start']}")
        return ""
    if count_overridden:
...
_LABEL_STYLES = (
    ...
    (("Last played:", "Now playing:", "Last track:", "Track:"), "track"),
    (("Playlist:",), "playlist"),
...
def find_song_in_playlists(song_name: str, previous_match, playlist_name):
    """Resolve which monitored playlist (if any) the current song belongs to.

    Checked in priority order:
      (A) Spotify itself reports a playlist name that matches a monitored one exactly. This is
          trusted immediately: if that playlist wasn't already about to be detected (count_start
          within one song of qty_start), its count is jump-started to qty_start - 1 (so the very
          next increment crosses the threshold) and 'previous_match', if any, is force-closed by
          setting its count_end to qty_end. Sets global count_overridden so the displayed count
          in build_playlist_status_message() accounts for the jump-start.
      (B) The song is in 'previous_match's own track list - stick with it for continuity (keeps a
          playlist "active" across a smart-shuffle song that Spotify doesn't attribute to it).
      (C) Fall back to scanning every monitored playlist's track list for the song.

    Returns the matched playlist's dict (a live reference into monitored_playlists_data, so the
    caller's mutations persist) or False if the song isn't in any monitored playlist.
    """
    ...
    for playlist_data in monitored_playlists_data.values():
        ...
        if playlist_name and (playlist_name.upper() == playlist_data.get('name', "B").upper()):
            ...
            if playlist_data['count_start'] < (playlist_data['qty_start'] - 1):
                ...
                if previous_match:
                    print_debug(f"*** OVERRIDE COUNT - END LAST PL  (A2): from {previous_match['count_end']} to {previous_match['qty_end']}")
                else:
                    print_debug(f"*** OVERRIDE COUNT END SKIPPED - CURRENT PLAYLIST IS FALSE - (A3): {previous_match}")
                playlist_data['count_start'] = playlist_data['qty_start'] - 1 # gets incremented after return
                count_overridden = True
                if previous_match:
                    previous_match['count_end'] = previous_match['qty_end']
    ...
    if previous_match:
        ...
        tracks_set = previous_match.get('tracks_set', False)
        if tracks_set and isinstance(tracks_set, set):
            if song_name_upper in tracks_set:
                print_debug(f"EXISTING PLAYLIST PRIORITY MATCHED!!! (B) : {previous_match.get('name', 'Missing')}, {song_name}")
...
def is_playlist_already_monitored_by_name(reported_playlist_name, monitored_playlists_data):
    """True if Spotify's reported playlist name exactly matches one we already monitor by name.

    Used to downgrade a search_playlist() 'hasTrack' hit: if the reported playlist IS a monitored
    one, its membership should be settled by our own track-list/count logic (find_song_in_playlists
    and friends), not by treating Spotify's report as authoritative.
    """
    return any(
        reported_playlist_name and reported_playlist_name.upper() == playlist_data.get('name', "B").upper()
        for playlist_data in monitored_playlists_data.values()
    )


def resolve_playlist_match(song_lookup_key, current_match, reported_playlist_name, has_track, playlist_owner, debug_tag, track_previous=True):
    """Shared "look up the monitored playlist for this song, unless we can't trust the lookup" step.

    Looked up via find_song_in_playlists() unless has_track is already True (Spotify itself
    confirmed the song is in the reported playlist, so there's nothing to resolve) or
    playlist_owner == "Spotify" (a Spotify-curated playlist, which songs get shuffled in/out of
    freely - not something to match against a fixed monitored track list).

    When track_previous is True (the normal case), the caller's current_match is captured as the
    new previous_match before the lookup runs, and both are reset to None together if the lookup is
    skipped. Some call sites intentionally leave previous_match untouched (it was already set moments
    earlier in the same iteration) - pass track_previous=False there and ignore the returned value.

    Returns (new_current_match, new_previous_match).
    """
    previous_match = current_match if track_previous else None
    if not has_track and (playlist_owner != "Spotify"):
        current_match = find_song_in_playlists(song_lookup_key, current_match, reported_playlist_name)
    else:
        current_match = None
        if track_previous:
            previous_match = None
        print_debug(f"SKIPPED FIND_SONG_IN_PLAYLIST ({debug_tag}) -> hasTrack: {has_track}")
...
def periodic_load_tracks_flexible(playlist_info):
    ...
    playlist_info['refresh'] (falling back to LOAD_TRACKS_FREQUENCY) seconds; refresh <= 0 disables
    reloading after this initial load.
    """
...
# A value placeholder is an upper-case metavar, a choice list or an angle-bracket name, including the
# colon-joined pair the OAuth credentials option takes
...
# Formats the songs-played count with the session length, omitted while the session is still on its first track
def songs_played_text(listened_songs: int, track_started_at: float, session_started_at: float) -> str:
    if session_started_at > 0 and int(track_started_at) != int(session_started_at):
        return f"{listened_songs} ({calculate_timespan(int(track_started_at), int(session_started_at))})"
...
def build_ntfy_image(image_url: str = "") -> Optional[bytes]:
    ...
    try:
        ...
        debug_print("NTFY image download", url=image_url)
        ...
        debug_print("NTFY image generation failed, sending text only", outcome="failed", error=f"{type(error).__name__}: {error}")
        return None
...
def debug_monitor_check_start(check_number: int, user: str, started_at: Optional[datetime] = None) -> datetime:
    ...
    debug_print("Starting check", check=f"#{check_number}", user=user)
...
def reload_secrets_signal_handler(sig, frame):
    ...
    if auth_values_after != auth_values_before:
        SP_ACTIVITY_METADATA_CACHE.clear()
        SP_CACHED_ACCESS_TOKEN = None
        SP_CACHED_REFRESH_TOKEN = None
        SP_ACCESS_TOKEN_EXPIRES_AT = 0
        SP_CACHED_CLIENT_ID = ""
...
# Checks token access through the selected activity source or the legacy metadata API
def check_token_validity(access_token: str, client_id: Optional[str] = None, user_agent: Optional[str] = None, oauth_app: Optional[bool] = False) -> bool:
    ...
    url = url2 if oauth_app else spotify_activity_endpoint()[0]
    ...
    try:
        if oauth_app:
            debug_print("HTTP GET", url=url, context="token validity", mode=check_mode, headers=sanitize_debug_headers(headers))
            response = req.get(url, headers=headers, timeout=FUNCTION_TIMEOUT, verify=VERIFY_SSL)
        else:
            response = spotify_request_activity(access_token, req, client_id, user_agent)
        valid = response.status_code == 200 or bool(oauth_app and response.status_code == 403)
        debug_print("Token validity", url=url, status=response.status_code, mode=check_mode, valid=valid)
    except Exception:
        valid = False
        debug_print("Token validity", url=url, mode=check_mode, outcome="failed")
    finally:
...
def refresh_access_token_from_sp_dc(sp_dc: str) -> dict:
    ...
    try:
        ...
        transport = False
        last_err = str(e)
        debug_print("HTTP GET", url=TOKEN_URL, context="sp_dc transport", outcome="failed", error=f"{type(e).__name__}: {e}")
    finally:
        _restore_timeout_alarm(alarm_state)

    if not transport or (sp_dc and (data.get("isAnonymous") is True or not check_token_validity(token, data.get("clientId", ""), USER_AGENT))):
        params["reason"] = "init"
        ...
        try:
            ...
            init = False
            last_err = str(e)
            debug_print("HTTP GET", url=TOKEN_URL, context="sp_dc init", outcome="failed", error=f"{type(e).__name__}: {e}")
        finally:
            _restore_timeout_alarm(alarm_state)
    ...
    # The live activity endpoint also accepts anonymous tokens, so its HTTP status cannot validate a cookie
    if sp_dc and data.get("isAnonymous") is True:
        raise ValueError("Spotify returned an anonymous token for the supplied sp_dc cookie")
...
def spotify_exchange_scrobble_authorization_code(client_id: str, redirect_uri: str, code: str, verifier: str, session: Optional[req.Session] = None) -> dict:
    ...
    debug_print("HTTP POST", url=SPOTIFY_SCROBBLE_TOKEN_URL, context="scrobble health PKCE token exchange")
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            if attempt + 1 >= max_attempts:
                raise
            debug_print("Spotify recent-play token refresh", outcome="failed", retry_in=display_time(SCROBBLE_HEALTH_IMMEDIATE_RETRY_DELAY), error=sanitize_error_text(exc))
            time.sleep(SCROBBLE_HEALTH_IMMEDIATE_RETRY_DELAY)
            continue
        if 500 <= response.status_code <= 599 and attempt + 1 < max_attempts:
            debug_print("Spotify recent-play token refresh", status=response.status_code, retry_in=display_time(SCROBBLE_HEALTH_IMMEDIATE_RETRY_DELAY))
            response.close()
            time.sleep(SCROBBLE_HEALTH_IMMEDIATE_RETRY_DELAY)
            continue
        return response
...
def spotify_get_recent_plays(client_id: Optional[str] = None, refresh_token: Optional[str] = None, session: Optional[req.Session] = None) -> List[SpotifyPlay]:
    ...
    for attempt in range(2):
        ...
        headers = {"Authorization": f"Bearer {access_token}", "User-Agent": USER_AGENT}
        debug_print("HTTP GET", url=SPOTIFY_SCROBBLE_RECENT_URL, context="scrobble health recent plays")
...
def lastfm_get_recent_scrobbles(username: str, api_key: str, session: Optional[req.Session] = None) -> List[LastfmScrobble]:
    ...
    debug_print("HTTP GET", url=url, context="Last.fm recent scrobbles", user=f"{username!r}")
...
def spotify_get_access_token_from_client(device_id, system_id, user_uri_id, refresh_token, client_token):
    ...
    try:
        ...
        debug_print("HTTP POST", url=LOGIN_URL, context="client auth", outcome="failed", reason="timeout", error=f"{type(e).__name__}: {e}")
        raise Exception(f"spotify_get_access_token_from_client() network request timeout after {display_time(FUNCTION_TIMEOUT + 2)}: {e}")
    except Exception as e:
        debug_print("HTTP POST", url=LOGIN_URL, context="client auth", outcome="failed", error=f"{type(e).__name__}: {e}")
...
def spotify_get_client_token(app_version, device_id, system_id, **device_overrides):
    ...
    try:
        ...
        debug_print("HTTP POST", url=CLIENTTOKEN_URL, context="client token", outcome="failed", reason="timeout", error=f"{type(e).__name__}: {e}")
        raise Exception(f"spotify_get_client_token() network request timeout after {display_time(FUNCTION_TIMEOUT + 2)}: {e}")
    except Exception as e:
        debug_print("HTTP POST", url=CLIENTTOKEN_URL, context="client token", outcome="failed", error=f"{type(e).__name__}: {e}")
...
def spotify_normalize_listening_activity(payload):
    if not isinstance(payload, dict) or not isinstance(payload.get("entities"), list):
        raise ValueError("Spotify listening activity response is missing entities")
    friends = {}
    for item in payload["entities"]:
        if not isinstance(item, dict):
            raise ValueError("Spotify listening activity entity is malformed")
        entity = item.get("userEntity", item.get("followEntity"))
        if entity is None:
            continue
        if not isinstance(entity, dict):
            raise ValueError("Spotify listening activity user is malformed")
        uri = entity.get("uri")
        if not isinstance(uri, str) or not uri.startswith("spotify:user:"):
            continue
        user_id = resolve_target_user_id(uri, None)
        if not user_id:
            raise ValueError("Spotify listening activity user URI is malformed")
        activity = entity.get("activity")
        if activity is None:
            continue
        if not isinstance(activity, dict):
            raise ValueError("Spotify listening activity is malformed")
        track_uri = activity.get("entityUri")
        ...
        if not re.fullmatch(r"spotify:track:[A-Za-z0-9]{22}", track_uri):
            raise ValueError("Spotify listening activity track URI is malformed")
        ...
        try:
            ...
            raise ValueError("Spotify listening activity timestamp is malformed") from exc
        is_playing = activity.get("isPlaying", False)
        if not isinstance(is_playing, bool):
            raise ValueError("Spotify listening activity playback state is malformed")
        ...
        if uri not in friends or timestamp_ms > friends[uri]["timestamp"]:
            friends[uri] = friend
    if len(payload["entities"]) >= SPOTIFY_ACTIVITY_RESULT_LIMIT:
        verbose_notice(f"Listening activity reached the {SPOTIFY_ACTIVITY_RESULT_LIMIT}-entry request limit. Older activity may be absent")
...
def spotify_get_friends_json(access_token):
    ...
    if not isinstance(friends_json, dict):
        raise ValueError("Spotify activity response is malformed")
...
def spotify_activity_metadata(kind, uri, access_token):
    key = (kind, uri)
    cached = SP_ACTIVITY_METADATA_CACHE.get(key)
    ...
    try:
        if kind == "user":
            user_id = resolve_target_user_id(uri, None)
            if not user_id:
                raise ValueError("Spotify activity metadata user URI is malformed")
            url = f"https://spclient.wg.spotify.com/user-profile-view/v3/profile/{quote(user_id, safe='')}"
            headers = {"Authorization": f"Bearer {access_token}", "User-Agent": USER_AGENT}
            if TOKEN_SOURCE == "cookie" and SP_CACHED_CLIENT_ID:
                headers["Client-Id"] = SP_CACHED_CLIENT_ID
            response = SESSION.get(url, params={"playlist_limit": 0, "artist_limit": 0, "episode_limit": 0, "market": "from_token"}, headers=headers, timeout=FUNCTION_TIMEOUT, verify=VERIFY_SSL, allow_redirects=False)
            response.raise_for_status()
            info = response.json()
        ...
        if not isinstance(name, str) or not name:
            raise ValueError("Spotify activity metadata has no name")
    except Exception as exc:
        ...
        if isinstance(exc, TimeoutException):
            raise
        debug_print("Activity metadata", kind=kind, outcome="unavailable", error_type=type(exc).__name__)
        return ""
    if len(SP_ACTIVITY_METADATA_CACHE) >= 256:
        SP_ACTIVITY_METADATA_CACHE.pop(next(iter(SP_ACTIVITY_METADATA_CACHE)))
    SP_ACTIVITY_METADATA_CACHE[key] = (time.monotonic() + 300, name)
    return name


# Returns a playlist name through Spotify's playlist service, or an empty string when the service does not answer with one
def spotify_get_playlist_name_spclient(playlist_uri, access_token):
    playlist_id = playlist_uri.split(":", 2)[2]
    url = f"{SPOTIFY_PLAYLIST_METADATA_URL}/{quote(playlist_id, safe='')}/metadata"
    headers = {"Authorization": f"Bearer {access_token}", "User-Agent": USER_AGENT, "Accept": "application/json"}
    if TOKEN_SOURCE == "cookie" and SP_CACHED_CLIENT_ID:
        headers["Client-Id"] = SP_CACHED_CLIENT_ID
    debug_print("HTTP GET", url=url, context="playlist metadata", headers=sanitize_debug_headers(headers))
    try:
        response = SESSION.get(url, headers=headers, timeout=FUNCTION_TIMEOUT, verify=VERIFY_SSL, allow_redirects=False)
        debug_print("HTTP GET", url=url, context="playlist metadata", status=response.status_code)
        response.raise_for_status()
        payload = response.json()
    except Exception as exc:
        if is_too_many_open_files(exc) or isinstance(exc, TimeoutException):
            raise
        debug_print("Playlist metadata", outcome="unavailable", error_type=type(exc).__name__)
        return ""
...
def spotify_complete_live_activity(info, access_token, track):
    if "sp_is_playing" not in info:
        return
    info["sp_username"] = spotify_activity_metadata("user", "spotify:user:" + info["sp_uri"], access_token) or info["sp_uri"]
    for target, source in (("sp_artist", "sp_artist_name"), ("sp_track", "sp_track_name"), ("sp_album", "sp_album_name"), ("sp_album_uri", "sp_album_uri")):
        info[target] = track.get(source) or ""
    context = info["sp_playlist_uri"]
    if context.startswith("spotify:playlist:"):
        info["sp_playlist"] = spotify_activity_metadata("playlist", context, access_token) or context
    elif context == info["sp_album_uri"]:
...
class LivePlaybackTiming:
    # Local time of the last sample, used to detect unobserved intervals
    sampled_at: Optional[float] = None
    # Activity timestamp reported by the feed in the last sample
    source_ts: Optional[float] = None
    playing: bool = False
    # Moment the open playing segment began, on the same clock as the feed timestamps
    segment_started_at: Optional[float] = None
    paused_at: Optional[float] = None
    track_started_at: float = 0
    session_started_at: float = 0
    track_seconds: float = 0
    segment_seconds: float = 0
    paused_seconds: float = 0
    pauses: int = 0
    start_observed: bool = False
    # False once any boundary of the current track had to fall back to a local sample time
    precise: bool = True
    # Whether the last reported change was timed with a trusted feed timestamp
    ...
    def continuous(self, now: float, max_gap: float) -> bool:
        return self.sampled_at is not None and self.sampled_at <= now <= self.sampled_at + max_gap

    # Chooses when a reported change happened, trusting the feed timestamp only when it is newer than the previous one and not later than the local sample
    def _event_time(self, now: float, source_ts: Optional[float], fallback: float) -> float:
        earliest = self.segment_started_at if self.playing else self.paused_at
        if source_ts is not None and self.source_ts is not None and self.source_ts < source_ts <= now and (earliest is None or source_ts >= earliest):
    ...
    # Credits the open playing segment up to the given moment
    def _close_segment(self, at: float) -> None:
        if self.segment_started_at is not None:
            played = max(0, at - self.segment_started_at)
    ...
    # Playing time of the current track including its open segment
    def played_seconds(self, now: float) -> float:
...
def spotify_list_friends(friend_activity, access_token):
    ...
    for friend in friend_activity["friends"]:
        ...
        if "isPlaying" in friend:
            _, info = spotify_get_friend_info({"friends": [friend]}, sp_uri)
            track_info = spotify_get_track_info(access_token, info["sp_track_uri"])
            spotify_complete_live_activity(info, access_token, track_info)
            friend["user"]["name"] = info["sp_username"]
            friend["track"]["name"] = info["sp_track"]
            friend["track"]["artist"]["name"] = info["sp_artist"]
            friend["track"]["album"].update({"name": info["sp_album"], "uri": info["sp_album_uri"]})
            friend["track"]["context"]["name"] = info["sp_playlist"]
...
def spotify_get_friend_info(friend_activity, uri):
    for friend in friend_activity["friends"]:
        ...
        if sp_uri == uri:
            ...
            info = {"sp_uri": sp_uri, "sp_username": sp_username, "sp_artist": sp_artist, "sp_track": sp_track, "sp_track_uri": sp_track_uri, "sp_track_uri_id": sp_track_uri_id, "sp_album": sp_album, "sp_album_uri": sp_album_uri, "sp_playlist": sp_playlist, "sp_playlist_uri": sp_playlist_uri, "sp_ts": sp_ts}
            if "isPlaying" in friend:
                info["sp_is_playing"] = friend["isPlaying"]
            return True, info
    return False, {}
...
def spotify_discover_web_query_hash(operation_name, force=False):
    ...
    debug_print("HTTP GET", url=WEB_PLAYER_URL, context="query discovery", operation=operation_name, headers=sanitize_debug_headers(headers))
    response = SESSION.get(WEB_PLAYER_URL, headers=headers, timeout=FUNCTION_TIMEOUT, verify=VERIFY_SSL)
    debug_print("HTTP GET", url=WEB_PLAYER_URL, context="query discovery", operation=operation_name, status=response.status_code)
    response.raise_for_status()
    ...
    debug_print("HTTP GET", url=bundle_url, context="query bundle", operation=operation_name)
    ...
    debug_print("HTTP GET", url=bundle_url, context="query bundle", operation=operation_name, status=bundle_response.status_code)
    ...
    debug_print("Persisted query hash discovered", operation_name=operation_name, url=bundle_url)
    return query_hash
...
def spotify_web_follow_operation(access_token, operation_name, variables):
    ...
    for attempt in range(2):
        ...
        debug_print("HTTP POST", url=WEB_PLAYER_QUERY_URL, context="follow", operation=operation_name, headers=sanitize_debug_headers(headers))
        response = SESSION.post(WEB_PLAYER_QUERY_URL, headers=headers, json=payload, timeout=FUNCTION_TIMEOUT, verify=VERIFY_SSL)
        debug_print("HTTP POST", url=WEB_PLAYER_QUERY_URL, context="follow", operation=operation_name, status=response.status_code)
        try:
        ...
        if errors and attempt == 0 and any(marker in error_message.lower() for marker in ("persistedquery", "persisted query", "sha256")):
            ...
            debug_print("Persisted query rejected", operation_name=operation_name, action="rediscovering its hash once")
            continue
        if errors:
...
def spotify_web_metadata_query(operation_name, variables, metadata_type):
    ...
    for attempt in range(2):
        ...
        debug_print("HTTP POST", url=WEB_PLAYER_QUERY_URL, context="web metadata", operation=operation_name, headers=sanitize_debug_headers(headers))
        response = SESSION.post(WEB_PLAYER_QUERY_URL, headers=headers, json=payload, timeout=FUNCTION_TIMEOUT, verify=VERIFY_SSL)
        debug_print("HTTP POST", url=WEB_PLAYER_QUERY_URL, context="web metadata", operation=operation_name, status=response.status_code)
...
def _spotify_get_playlist_owner_and_image_api(access_token, playlist_uri, oauth_app=False):
    ...
    debug_print("HTTP GET", url=url, context="legacy playlist owner", headers=sanitize_debug_headers(headers))
    response = SESSION.get(url, headers=headers, timeout=FUNCTION_TIMEOUT, verify=VERIFY_SSL)
    debug_print("HTTP GET", url=url, context="legacy playlist owner", status=response.status_code)
    response.raise_for_status()
    json_response = response.json()
...
def spotify_get_playlist_owner_and_image(access_token, playlist_uri, oauth_app=False):
    ...
    try:
        ...
        if api_status == 404:
            ...
            if spotify_owned:
                SP_WEB_PLAYLIST_API_FAILURES = 0
                debug_print("spotify_get_playlist_owner_and_image(): legacy Web API hides Spotify-curated playlist", uri=f"{playlist_uri}, using the web-player backend for this playlist")
            elif owner_name or owner_uri:
                SP_WEB_PLAYLIST_API_FAILURES += 1
                SP_WEB_PLAYLIST_BACKEND_PREFERRED = True
                debug_print("spotify_get_playlist_owner_and_image(): legacy Web API returned 404 for non-Spotify playlist", uri=f"{playlist_uri}, preferring the web-player backend for remaining playlists")
                verbose_notice("Playlist metadata switched to the web-player backend after a legacy API restriction")
            else:
                debug_print("spotify_get_playlist_owner_and_image(): legacy Web API returned 404 for playlist", uri=f"{playlist_uri}, using the web-player backend for this playlist")
        debug_print("Playlist image", url=playlist_image_url)
...
def _spotify_get_track_info_api(access_token, track_uri, oauth_app=False):
    ...
    debug_print("HTTP GET", url=url, context="legacy track info", headers=sanitize_debug_headers(headers))
    response = SESSION.get(url, headers=headers, timeout=FUNCTION_TIMEOUT, verify=VERIFY_SSL)
    debug_print("HTTP GET", url=url, context="legacy track info", status=response.status_code)
    response.raise_for_status()
    json_response = response.json()
    ...
    if api_available and not SP_WEB_TRACK_BACKEND_PREFERRED:
        try:
            ...
            if spotify_should_latch_web_backend(error, SP_WEB_TRACK_API_FAILURES):
                ...
                verbose_notice("Track metadata switched to the web-player backend after legacy API failures")
            else:
...
def doctor_notification_images_detail() -> str:
    if NTFY_IMAGES:
        return "NTFY_IMAGES is enabled, so ntfy alerts are sent as text only until Pillow is installed. Every other feature is unaffected"
    return "Required only when NTFY_IMAGES attaches artwork to ntfy alerts, which is currently disabled. Every other feature is unaffected"
    ...
    for module_name, package_name, purpose in optional:
        ...
        if present:
            ...
            if module_name == "pycookiecheat":
                missing_purpose = "Required only for importing cookies from Chromium-based browsers. Every other feature is unaffected. Firefox cookie import is also unaffected"
            elif module_name == "PIL":
                missing_purpose = doctor_notification_images_detail()
                missing_fix = doctor_notification_images_fix()
            elif module_name == "colorama":
                missing_purpose = "Coloured output may not render in the classic Windows Command Prompt. Every other feature is unaffected"
                missing_fix = "Install it with: pip3 install colorama. Or use Windows Terminal, which needs nothing extra"
            else:
                missing_purpose = f"Optional: {purpose}. Every other feature is unaffected when this feature is unused"
...
BUILT_IN_SHAPE_SETTINGS = {name: globals()[name] for name in ('SP_LOGFILE', 'CSV_FILE', 'MONITOR_LIST_FILE', 'DOTENV_FILE', 'COLOR_THEME', 'TRUNCATE_CHARS', 'SP_APP_TOKENS_FILE', 'LOGIN_REQUEST_BODY_FILE', 'CLIENTTOKEN_REQUEST_BODY_FILE', 'FLAG_FILE', 'SCROBBLE_HEALTH_STATE_FILE') if name in globals()}
...
def command_reports_configuration(args=None):
    ...
    return any(getattr(args, name, False) for name in ("doctor", "setup", "set_lastfm_credentials", "set_smtp_password", "set_sp_dc", "set_webhook_url"))
...
def prepare_configured_paths(args):
    ...
    overrides.update({'LOGIN_REQUEST_BODY_FILE': 'login_request_body_file', 'CLIENTTOKEN_REQUEST_BODY_FILE': 'clienttoken_request_body_file', 'FLAG_FILE': 'flag_file'})
    settings = globals().copy()
...
def configuration_shape_errors(settings=None):
    ...
    for name in ('SP_LOGFILE', 'CSV_FILE', 'MONITOR_LIST_FILE', 'DOTENV_FILE', 'SP_APP_TOKENS_FILE', 'LOGIN_REQUEST_BODY_FILE', 'CLIENTTOKEN_REQUEST_BODY_FILE', 'FLAG_FILE', 'SCROBBLE_HEALTH_STATE_FILE'):
...
def doctor_check_configuration(config_path=None, env_path=None, startup_checks: Sequence[DoctorCheck] = (), target_value=None, lastfm_username=None) -> List[DoctorCheck]:
    ...
    if MONITOR_MODE != "scrobble_health":
        try:
            endpoint, method = spotify_activity_endpoint()
            checks.append(make_doctor_check("Configuration", "PASS", f"Friend Activity backend is {FRIEND_ACTIVITY_BACKEND}", f"{method} {endpoint}"))
        except ValueError as exc:
            advice = classify_recovery_error(exc, "config_invalid")
            checks.append(make_doctor_check("Configuration", "FAIL", "FRIEND_ACTIVITY_BACKEND is invalid", advice.detail, advice))
...
# Validates configured Spotify credentials and returns reusable activity data
def doctor_check_authentication(report: DoctorReport) -> List[DoctorCheck]:
    ...
    try:
        spotify_activity_endpoint()
    except ValueError as exc:
        advice = classify_recovery_error(exc, "config_invalid")
        report.authentication_advice = advice
        return [make_doctor_check("Authentication", "SKIP", "Correct the Friend Activity backend before authentication", advice.detail, advice)]
    try:
...
# Validates an optional target against the selected activity source
...
def doctor_check_optional_oauth() -> List[DoctorCheck]:
    ...
    try:
        ...
        if is_too_many_open_files(legacy_error):
            print_recovery_advice(classify_recovery_error(legacy_error))
            raise SystemExit(1)
        try:
...
def _wizard_collect_scrobble_health_auth_section(state: ScrobbleHealthSetupState, method: str) -> None:
    ...
    while True:
        ...
        try:
            ...
            print(f"  {exc}.")
            if not _wizard_offer_retry("Spotify app Client ID"):
                client_id = ""
                break
    ...
    authorize_now = True
    if not client_id:
        # Authorization belongs to one Spotify app, so an abandoned Client ID finishes setup incomplete instead of looping
        authorize_now = False
    elif existing_matches_app:
...
def run_setup_wizard(initial_target: Optional[str] = None, config_file=None, env_file=None) -> None:
    ...
    try:
        ...
        if doctor_offered and _wizard_ask_yes_no("Run doctor now? It writes no files and offers real delivery tests only with separate approval.", default=True):
            doctor_ran = True
            if _wizard_load_effective_setup(config_path, env_path):
                doctor_failed = run_doctor(target, str(config_path), str(env_path) if env_path.is_file() else None) != 0
                auth["validated"] = not doctor_failed
            else:
                doctor_failed = True
    except (EOFError, KeyboardInterrupt):
...
def notify_playlist_detected(notify_playlist, songstr, timediff, track, artist, album):
    """Fire the "playlist detected" email/spreadsheet/push notification, gated on the playlist's 'notify' setting.

    Always returns a screen-only status line ("*** Playlist 'X' Detected"). The alert text sent to
    email/push (a separate, slightly different string) is only built and sent when
    notify_playlist.get('notify', NOTIFY_PLAYLIST_DETECTED) is True.
    """
    playlist_screen_message = f"{timestring()}: {ERR_CODE}, [{timediff}] *** Playlist '{notify_playlist['name']}' Detected"
    if notify_playlist.get('notify', NOTIFY_PLAYLIST_DETECTED):
        ...
        playlist_message = f"*** Playlist '{notify_playlist['name']}' Detected: {songstr}"
        send_notification("detected", playlist_message, "", track, artist, album, notify_playlist['name'], "", 0)
    return playlist_screen_message


def notify_playlist_cleared(notify_playlist, songstr, timediff, track, artist, album):
    """Fire the "playlist cleared" notification and return (playlist_message, playlist_screen_message).
...
def monitored_playlist_detected(detected_playlist, songstr, timediff, print_msg, track="", artist="", album=""):
    ...
    """
    msg = build_playlist_status_message(detected_playlist)
    if DEBUG_JMK:
        msg = msg + " (1)"
...
def monitored_playlist_cleared(cleared_playlist, songstr, timediff, track="", artist="", album=""):
    """Thin pass-through to notify_playlist_cleared(); kept separate to mirror monitored_playlist_detected()'s name."""
    playlist_message, playlist_screen_message = notify_playlist_cleared(cleared_playlist, songstr, timediff, track, artist, album)
    return playlist_message, playlist_screen_message
...
def run_scrobble_health_setup_wizard(config_file=None, env_file=None) -> None:
    ...
    try:
        ...
        state = ScrobbleHealthSetupState(config_path, env_path, baseline_values, config_values, {}, str(baseline_values.get("LASTFM_USERNAME") or ""), initial_auth, [], [])
        _wizard_collect_scrobble_health_profile_section(state)
        _wizard_collect_scrobble_health_threshold_section(state)
        _wizard_collect_scrobble_health_auth_section(state, method)
        print()
...
def spotify_monitor_friend_uri(user_uri_id, tracks, csv_file_name):
    ...
    # --- Monitored-playlist detection state for this friend ---
    # current_match / previous_match are live references into monitored_playlists_data (or None):
    # the playlist the *current* song was just matched to, and the one the *previous* song was
    # matched to. Comparing the two is what drives the "did we switch playlists" / "is this still
    # the same playlist" logic below. Each dict has qty_start/qty_end (from ADD_PLAYLISTS_TO_MONITOR)
    # and count_start/count_end/count_shuffle (running counters, see reset_playlist_counts()).
    current_match = None
    previous_match = None
    # playlist_message / playlist_screen_message / playlist_body_text / playlist_body_html are the
    # "playlist detected/cleared" status text for the current song, in four presentations: plain
    # text for logging, a screen-only line, and text/HTML fragments for the email body. Rebuilt (or
    # cleared) on every song change - see build_playlist_status_message(), notify_playlist_detected(),
    # notify_playlist_cleared(), and reset_playlist_counts() below.
    playlist_message = ""
    playlist_screen_message = ""
    playlist_body_text = ""
    playlist_body_html = ""
    ...
    def reset_playlist_counts(playlist_name_to_protect=""):
        ...
        If playlist_name_to_protect names a playlist, that one keeps its count_start/count_shuffle
        (only count_end is cleared) so a playlist that's mid-detection or mid-count doesn't lose its
        progress just because every *other* playlist is being reset around it. Every other playlist
        is fully zeroed. Also drops the ALT_VIEW "song missing from playlist" icon flag, since it's
        tied to the same "we just moved on" moment as the playlist counters.
        """
        nonlocal icon_add
        nonlocal playlist_message, playlist_body_text, playlist_body_html, playlist_screen_message
    ...
    while True:
        ...
        try:
            ...
            if error_email_pending or error_webhook_pending:
                email_succeeded, webhook_succeeded = send_notification_channels("error", m_subject, m_body, m_body_html, error_email_pending, error_webhook_pending)
                error_alert.record("email", error_email_pending, email_succeeded, now)
                error_alert.record("webhook", error_webhook_pending, webhook_succeeded, now)
        ...
        if sp_found:
            ...
            try:
                ...
                spotify_complete_live_activity(sp_data, sp_accessToken, sp_track_data)
                sp_album_image_url = sp_track_data.get("sp_album_image_url", "")
                debug_print("Album image", url=sp_album_image_url)
...
# Playlist-detection snapshot for whatever song is playing right when this friend is (re)acquired
# from the friends-list poll - i.e. the outer "while True:" loop above (re)starting, whether that's
# the very first check of the process or a friend reappearing after having dropped off the list.
# Silent by design: this only recomputes state and the printable playlist_message; it never fires
# an email/push notification (see notify_playlist_detected/cleared) - those happen once a real
# per-song-change is observed in the primary loop below (LOOP C).
# ------------------------------------------------------------
            ...
            current_match, previous_match = resolve_playlist_match(song_lookup_key, current_match, sp_playlist if is_playlist else "", hasTrack, sp_playlist_owner, "1A")
            ...
            if current_match and previous_match and (current_match.get('name', 'A') != previous_match.get('name', 'B')):
...
#jmkfix can this case every happen. is it in log?
                if previous_match['count_end'] >= (previous_match['qty_end']):
                    new_playlist = True
                    print_debug(f"SPECIAL CASE: SONG IN ANOTHER MONITORED PLAYLIST, AT EXCEPTION LIMIT FOR CURRENT (1) - old: {previous_match.get('name', 'A')} new: {current_match.get('name', 'A')}")
...
#jmkfix can this case every happen. is it in log?
            ...
            if current_match and new_playlist:
                print_debug(f"FOUND PLAYLIST IN MONITORING LIST (1) -> {current_match['name']}")
                print_debug(f"COUNT START: {current_match['count_start']}, {current_match['qty_start']}")
                reset_playlist_counts(current_match['name'] if current_match else "")
                current_match['count_start'] += 1
                print_debug(f"COUNT START +1: {current_match['count_start']}")
                if current_match.get('override', OVERRIDE_PLAYLIST_AT_START):
                    print_debug(f"FIRST BOOT PLAYLIST COUNT OVERRIDE")
                    print_debug(f"OVERRIDE COUNT (1): from {current_match['count_start']} to {current_match['qty_start']}")
                    if current_match['count_start'] < current_match['qty_start']:
                        current_match['count_start'] = current_match['qty_start']
                        count_overridden = True
                if current_match['count_start'] >= current_match['qty_start']:
                    print_debug(f"PLAYLIST_DETECTED (1): {current_match['count_start']}, {current_match['qty_start']}")
                    is_playlist = True
                    sp_playlist = current_match['name']
                    sp_playlist_url = current_match.get('url', '')
                    sp_track = sp_track + current_match.get('icon', '')
                ...
                playlist_message = build_playlist_status_message(current_match)
                if DEBUG_JMK and playlist_message != "":
                    playlist_message = playlist_message + " (2)"
                playlist_body_text = playlist_message + "\n"
                playlist_body_html = playlist_message + "<br>"

            else:
                if DEBUG_JMK and not current_match:
                    print_debug(f"SONG NOT IN A MONITORED PLAYLIST (1)")

                # Process a possible exception if last song was in a playlist (even if as exception) AND
                # it's an active playlist (not counting up towards it). Smart shuffle can put a song not
                # on the list into the middle of a run, so a single non-matching song isn't necessarily
                # the end of the playlist - only qty_end of them in a row is.
                if previous_match:
                    print_debug(f"COUNT END: {previous_match['count_end']}")
                    previous_match['count_end'] += 1
                    print_debug(f"COUNT END + 1: {previous_match['count_end']}")
                    if current_match:
                        current_match['count_start'] += 1
                        print_debug(f"COUNT START + 1 - {current_match['name']}: {current_match['count_start']}")
                    if previous_match['count_end'] >= previous_match['qty_end']:
                        reset_playlist_counts(current_match['name'] if current_match else "")
                        count_overridden = False
                    else:
                        # since haven't hit the limit yet to consider the playlist over, keep tracking it
                        ...
                        print_debug(f"COUNT_SHUFFLE: {current_match['count_shuffle']}")
                        current_match['count_shuffle'] += 1
                        print_debug(f"COUNT SHUFFLE + 1: {current_match['count_shuffle']}")
                        # don't show icon in this case, but OK to show playlist with an *
                        if ALT_VIEW:
                            icon_add = True
                        print_debug(f"HAVEN'T HIT LIMIT TO DISCONTINUE PLAYLIST (1) -> {current_match['name']}")
                else:
                    reset_playlist_counts()
                    count_overridden = False
...
#                print(f"Monitoring Tracks [alerts: {f'{str(playlist_data.get('notify')).lower()},':<6} refresh: {playlist_data.get('refresh'):>4}]: {playlist_name} ({len(playlist_data.get('tracks_set'))} songs)")
            ...
            if playlist_message or listened_songs:
                print("")
            if playlist_message:
                print(playlist_message)
...
#                song_count = 1
                ...
                # ALT_VIEW-only: re-checks whether the track already playing when this friend was first
                # observed active this session is already deep enough into a monitored playlist to
                # announce immediately, without waiting for the next per-song check (LOOP C) to catch it.
                current_match, _ = resolve_playlist_match(song_lookup_key, current_match, sp_playlist if is_playlist else "", hasTrack, sp_playlist_owner, "1B", track_previous=False)
                print_debug(f"PLAYLIST CHECK: {current_match}, {song_lookup_key}")
                if current_match:
                    print_debug(f"PLAYLIST FOUND: count: {current_match['count_start']}")
                    if current_match['count_start'] >= current_match['qty_start']:
                        print_debug(f"BOOTUP PLAYLIST DETECTED")
                        print_debug(f"PLAYLIST_DETECT (1): {current_match['count_start']}")
                        playlist_body_text, playlist_body_html, playlist_message, playlist_screen_message = monitored_playlist_detected(current_match, songstring(), time_diff_str(), True, sp_track, sp_artist, sp_album)
                    else:
                        playlist_message = build_playlist_status_message(current_match)
                        if DEBUG_JMK and playlist_message != "":
                            playlist_message = playlist_message + " (3)"
...
#jmk 9/19/2026                    alive_counter = 0
#jmk 9/19/2026 #                    song_count += 1
...
# Playlist-detection state machine, run once per genuine track change (this is LOOP C, inside
# "if track_changed:") - the steady-state path that fires real detected/cleared notifications.
# This is the fullest, most-corrected version of the same logic sketched in LOOP A above.
                    ...
                    if not hasTrack:
                        current_match = find_song_in_playlists(song_lookup_key, current_match, sp_playlist if is_playlist else "")
                    else:
                        # playlist-cleared notification is lost here - 2/28/2026 try to fix #jmk
                        if active_ever and previous_match:
                            print_debug(f"previous_match: {previous_match}")
                            ...
                            playlist_message, playlist_screen_message = monitored_playlist_cleared(previous_match, songstring(), time_diff_str(), sp_track, sp_artist, sp_album)
                            print_debug(f"playlist_message: {playlist_message}, playlist_screen_message: {playlist_screen_message}")
                            if ALT_VIEW:
                                print_to_screen(playlist_screen_message) # could get overwritten in next section
                                playlist_screen_message = "" # prevent duplicate printing below
                    ...
                    if DEBUG_JMK:
                        if current_match:
                            print_debug(f"playlist A: {current_match.get('name', 'A')}")
                        if previous_match:
                            print_debug(f"playlist B: {previous_match.get('name', 'B')}")
                    if current_match and previous_match and (current_match.get('name', 'A') != previous_match.get('name', 'B')):
                    # if this song puts currently tracked playlist over the exception limit, then it can be start of a newly detected playlist
                        print_debug(f"count_end: {previous_match['count_end']}, qty_end: {previous_match['qty_end']}")
                        if previous_match['count_end'] >= (previous_match['qty_end']):
                            new_playlist = True
                            print_debug(f"SPECIAL CASE: SONG IN ANOTHER MONITORED PLAYLIST, AT EXCEPTION LIMIT FOR CURRENT (2) - old: {previous_match.get('name', 'A')} new: {current_match.get('name', 'A')}")
                    ...
                    # else it should be considered an exception
                            ...
                            print_debug(f"SPECIAL CASE: SONG EXCEPTION BUT IN ANOTHER MONITORED PLAYLIST (2): count_end: {previous_match['count_end']} qty_end: {previous_match['qty_end']}")
                            new_playlist = False
                    else:
                        new_playlist = True
                    ...
                    # if playlist changed, need to handle it as an exception first
                    if current_match and new_playlist:
                        print_debug(f"FOUND PLAYLIST IN MONITORING LIST (2) -> {current_match['name']}")
                        print_debug(f"COUNT START: {current_match['count_start']}, {current_match['qty_start']}")
                        reset_playlist_counts(current_match['name'] if current_match else "")
                        current_match['count_start'] += 1
                        print_debug(f"COUNT START + 1: {current_match['count_start']}")
                        ...
                        if current_match['count_start'] >= current_match['qty_start']:
                            print_debug(f"PLAYLIST_DETECTED (2): {current_match['count_start']}, {current_match['qty_start']}")
                            is_playlist = True
                            sp_playlist = current_match['name']
                            sp_playlist_url = current_match.get('url', '')
                            sp_track = sp_track + current_match.get('icon', '')
                        ...
                        if not ((cur_ts - sp_ts_old) > SPOTIFY_INACTIVITY_CHECK and sp_active_ts_stop > 0):
                            ...
                            if current_match['count_start'] == current_match['qty_start']:
                                print_debug(f"PLAYLIST_DETECT (2): {current_match['count_start']}")
                                playlist_body_text, playlist_body_html, playlist_message, playlist_screen_message = monitored_playlist_detected(current_match, songstring(), time_diff_str(), False, sp_track, sp_artist, sp_album) # False is because playlist_screen_message is printed below in sequence after the song name gets printed
                            else:
                            # not enough to start playlist
                                playlist_message = build_playlist_status_message(current_match)
                                if DEBUG_JMK and playlist_message != "":
                                    playlist_message = playlist_message + " (4a)"
                                    print_debug(f"PLAYLIST_MESSAGE (4a) -> {playlist_message}")
                        else:
                        # inactive user (but going active?)
                            playlist_message = build_playlist_status_message(current_match)
                            if DEBUG_JMK and playlist_message != "":
                                playlist_message = playlist_message + " (4b)"
                                print_debug(f"PLAYLIST_MESSAGE (4b) -> {playlist_message}")
...
# this check handles if NOT just becoming active
                        ...
                        #7/31/25 do we need an "is user just becoming active" check here? (LOOP A doesn't have it)
                        ...
                        if previous_match:
                            ...
                            print_debug(f"COUNT END - {previous_match['name']}: {previous_match['count_end']}")
                            previous_match['count_end'] += 1
                            print_debug(f"COUNT END + 1 - {previous_match['name']}: {previous_match['count_end']}")
                            if current_match:
                                current_match['count_start'] += 1
                                print_debug(f"COUNT START + 1 - {current_match['name']}: {current_match['count_start']}")
                            ...
                            if previous_match['count_end'] >= previous_match['qty_end']:
                                ...
                                # so only clear if the last playlist had actually been confirmed active
                                #jmkfix 08/04 this DEFINITELY also runs during runtime
                                if previous_match['count_start'] >= previous_match['qty_start']:
                                    playlist_message, playlist_screen_message = monitored_playlist_cleared(previous_match, songstring(), time_diff_str(), sp_track, sp_artist, sp_album)
                                ...
                                if previous_match['count_start'] >= previous_match['qty_start']:
                                    ...
                                    print_debug(f"SPECIAL CASE 2: PLAYLIST FOUND BUT PREVIOUS ONE WAS STILL COUNTING UP -> {current_match['name']}")

                                print_debug(f"HAVEN'T HIT LIMIT TO DISCONTINUE PLAYLIST (2) -> {current_match['name']}")
                        else:
                            reset_playlist_counts()
                            count_overridden = False
#jmk 2026/09/19                    listened_songs += 1
        ...
        # main song line printer is here
                        if ALT_VIEW:
                            print_to_screen(f"{timestring()}: {ERR_CODE}, [{time_diff_str()}] {songstring()}")
                            send_notification("song", f"{timestring()}: {ERR_CODE}, [{time_diff_str()}] {songstring()}", sp_album_image_url, sp_track, sp_artist, sp_album, (sp_playlist+iconstring()) if is_playlist else '', time_diff_str(), listened_songs)
                            if playlist_screen_message:
                                print_debug(f"PLAYLIST_SCREEN_MESSAGE: {playlist_screen_message}")
                                print_to_screen(playlist_screen_message)

                    print(f"Spotify user:\t\t\t{sp_username}")
                    print(f"\n{activity_label}:{activity_tabs}{sp_artist} - {sp_track}")
                    print(f"Duration:\t\t\t{display_time(sp_track_duration)}")

#jmk 2026/09/19                    # Suppress "Played for" if this track is the first after inactivity
#jmk 2026/09/19                    cur_ts = int(time.time())
#jmk 2026/09/19                    resumed_after_offline = (sp_active_ts_stop > 0) and ((cur_ts - sp_ts_old) > SPOTIFY_INACTIVITY_CHECK)
#jmk 2026/09/19                    song_skipped = False
#jmk 2026/09/19                    if not resumed_after_offline and (sp_ts - sp_ts_old) < (sp_track_duration - 1):
                    listened_songs += 1
...
#jmk                if 'spotify:album:' in sp_playlist_uri and sp_playlist != sp_album:
                    ...
                    # Live reports end with the timestamp alone, since the feed timestamp is the track start they already show
                    last_activity_m_body = "\n" if live_activity else f"\n\nLast activity: {get_date_from_ts(sp_ts)}"
                    last_activity_m_body_html = "<br>" if live_activity else f"<br><br>Last activity: <b>{get_date_from_ts(sp_ts)}</b>"
...
#jmk                if 'spotify:album:' in sp_playlist_uri and sp_playlist != sp_album:
                    ...
                    if resumed_live_session or (not live_activity and (cur_ts - sp_ts_old) > activity_inactivity_check() and sp_active_ts_stop > 0):
                        ...
                        if not live_activity and (sp_active_ts_start - sp_active_ts_stop) < 30:
                        ...
                        if ALT_VIEW:
                            ...
                            # below - it deliberately does NOT re-run the counting/exception logic, since doing so
                            # would double-count this song on top of what LOOP C already did.
                            current_match, previous_match = resolve_playlist_match(song_lookup_key, current_match, sp_playlist if is_playlist else "", hasTrack, sp_playlist_owner, "3")

                            if current_match:
                                print_debug(f"FOUND PLAYLIST IN MONITORING LIST (3) -> {current_match['name']}")
                                print_debug(f"COUNT START: {current_match['count_start']}, {current_match['qty_start']}")
                                if current_match['count_start'] >= current_match['qty_start']:
                                    print_debug(f"PLAYLIST_DETECTED (3): {current_match['count_start']}, {current_match['qty_start']}")
                                    is_playlist = True
                                    sp_playlist = current_match['name']
                                    sp_playlist_url = current_match.get('url', '')
                                    playlist_m_body = f"\nPlaylist: {sp_playlist}{iconstring()}"
                    ...
                    if song_on_loop == SONG_ON_LOOP_VALUE and ((SONG_ON_LOOP_NOTIFICATION and not email_sent) or (webhook_event_enabled("loop") and not webhook_sent)):
                        ...
                        m_body_html = f"<html><head></head><body>{escape(activity_label)}: <b><a href=\"{escape_html_attr(sp_artist_url)}\">{escape(sp_artist)}</a> - <a href=\"{escape_html_attr(sp_track_url)}\">{escape(sp_track)}</a></b><br>Duration: {display_time(sp_track_duration)}{played_for_m_body_html}{playlist_m_body_html}<br>Album: <a href=\"{escape_html_attr(sp_album_url)}\">{escape(sp_album)}</a>{context_m_body_html}{music_section_html}{lyrics_section_html}User plays song on LOOP (<b>{song_on_loop}</b> times)<br><br>Songs played: {songs_played_text(listened_songs, activity_ts, sp_active_ts_start)}{last_activity_m_body_html}{get_cur_ts('<br>Timestamp: ')}</body></html>"
                        m_body_short = build_short_ntfy_body(sp_track, sp_artist, sp_album, sp_playlist if is_playlist else "", playlist_suffix)
                    ...
                    if email_song_enabled or webhook_song_enabled:
                        ...
                        m_subject_short = build_short_ntfy_session_subject(sp_username, calculate_timespan(int(activity_ts), int(sp_active_ts_start), show_seconds=False, short=True), listened_songs)
                        m_body = f"{activity_label}: {sp_artist} - {sp_track}\nDuration: {display_time(sp_track_duration)}{played_for_m_body}{playlist_m_body}\nAlbum: {sp_album}{context_m_body}{music_section_text}{lyrics_section_text}Songs played: {songs_played_text(listened_songs, activity_ts, sp_active_ts_start)}{last_activity_m_body}{get_cur_ts(nl_ch + 'Timestamp: ')}"
                        m_body_html = f"<html><head></head><body>{escape(activity_label)}: <b><a href=\"{escape_html_attr(sp_artist_url)}\">{escape(sp_artist)}</a> - <a href=\"{escape_html_attr(sp_track_url)}\">{escape(sp_track)}</a></b><br>Duration: {display_time(sp_track_duration)}{played_for_m_body_html}{playlist_m_body_html}<br>Album: <a href=\"{escape_html_attr(sp_album_url)}\">{escape(sp_album)}</a>{context_m_body_html}{music_section_html}{lyrics_section_html}Songs played: {songs_played_text(listened_songs, activity_ts, sp_active_ts_start)}{last_activity_m_body_html}{get_cur_ts('<br>Timestamp: ')}</body></html>"
                        m_body_short = build_short_ntfy_body(sp_track, sp_artist, sp_album, sp_playlist if is_playlist else "", playlist_suffix)
                    ...
                    if live_activity:
                        ...
                        if playback_event and sp_active_ts_start > 0:
                            if playback_event == "paused":
                                print(f"User PAUSED playing after {display_time(int(playback_duration))}")
                    ...
                    if (cur_ts - last_activity_at) > activity_inactivity_check() and sp_active_ts_start > 0:
                        ...
                        if live_activity:
                            ...
                            if completed_pauses > 0 and live_timing.paused_seconds > 0 and sp_active_ts_stop > sp_active_ts_start:
                                paused_percentage = int(live_timing.paused_seconds / (sp_active_ts_stop - sp_active_ts_start) * 100)
...
def main():
    ...
    if not scrobble_health_mode and not args.doctor:
        try:
            spotify_activity_endpoint()
        except ValueError as exc:
            print_recovery_error(exc, "config_invalid")
    ...
    if args.import_browser_cookie:
        ...
        try:
            run_browser_cookie_import(browser=args.browser or "firefox", browser_profile=args.browser_profile, cookie_file=args.cookie_file, env_file=args.env_file, force=args.force, config_path=args.config_file, target=args.user_id, saved_target=TARGET_USER_URI_ID)
        except BrowserCookieImportError as exc:
    ...
    # It is dropped from the environment rather than only filtered here, since the loop that assigns the exported
    # values below reads os.getenv again and would otherwise overwrite what the config file or the dotenv file holds
    for environment_key in (*SECRET_KEYS, *ENVIRONMENT_SETTING_KEYS):
        if os.environ.get(environment_key) == "":
            os.environ.pop(environment_key)
